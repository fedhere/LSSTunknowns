            ...
            if len(dataSlice[self.m5Col][np.where(dataSlice['filter'][obs]==self.f)])>2:     
                ...
                for i, (dT, see, ra,fiveSlim) in enumerate(zip(deltaT,seeing,dataSlice['fieldRA'][obs],dataSlice[self.m5Col][obs])):
                    ...
                    if np.size(good)>0:
                        ...
                        #variables = {k:np.array(variables[k]) for k in names}                      
        ...
        def run(self, dataSlice, slicePoint=None): 
            ...
            if self.dataout: 
                ...
                if np.isnan(out[dm]['num']).all(): 
                    ...
                    alpha = N*len(LM[np.where(LM==0)])/len(DM[np.isfinite(DM)])#*weight[np.where(DM !=0)]/weight[np.where(DM !=0)].sum() 
        ...
        def run(self, dataSlice, slicePoint=None): 
            ...
            if len(dataSlice[self.m5Col][obs])>2: 
                ...
                for i, (dT, see, ra,fiveSlim) in enumerate(zip(deltaT,seeing,dataSlice['fieldRA'][obs],dataSlice[self.m5Col][obs])):
                        ...
                        #mode = self.position_selection(R,z)
                        mode = self.simobj['MODE']
                        
                        #V_matrix=np.vstack((self.U,self.V,self.W))
                        #velmat = np.empty_like(V_matrix[0,:])
                        #veloutmat = np.empty_like(V_matrix[0,:])
                        #Pv=self.DF(V_matrix,mode,R,z)  
                        #vel_idx=np.random.choice(np.arange(0,len(V_matrix[0,:]),1)[np.isfinite(Pv)],p=Pv[np.isfinite(Pv)]/np.sum(Pv[np.isfinite(Pv)]), size = 3)
                        #if self.unusual=='uniform':
                        #    vel_out= uniform(-1000,1000) 
                        #    veloutmat = vel_out.rvs(size=3)
                        #else:
                        #    vel_out = pd.read_csv(self.unusual)
                        #    veloutmat = np.random.choice(vel_out['vel'],p=vel_out['fraction']/np.sum(vel_out['fraction']),size=3)
                        ...
                        good = np.where((arc > 0.05*see) & (snr>self.snr_lim))[0]
                        if np.size(good)>0:
                           #variables['z'].append(z)
                            #variables['R'].append(R)
                ...
                #variables['MODE'] = np.concatenate(variables['MODE'])
                            if self.dataout: 
                                like_PM = self.Likelihood(variables, ty='usual') 
                                like_PMout = self.Likelihood(variables, ty='unusual') 
                                lam = {'L_PM':like_PM, 'L_PMout': like_PMout, 'mag':variables['mag'],'PM': variables['PM'], 'PM_OUT':variables['PM_out']}
                            #PM = {'PM': variables['PM'], 'PM_OUT':variables['PM_out']} 
                                return{'LS':lam} 
                            else: 
                                #unusual = 0
                                #mag_idx = np.where((g_dist['mag_g']>self.mag_lim[0])&(g_dist['mag_g']<self.mag_lim[1]))
                                #hist_mag, mag_edges = np.histogram(variables['mag'], bins=np.arange(self.mag_lim[1],self.mag_lim[0],1))
                                #for i in range(len(mag_edges)-1):
                                    #mag_index =  np.where((variables['mag']>mag_edges[i]) & (variables['mag']<mag_edges[i+1]))
                                if np.size(variables['PM']) >0:
                                    ...
                                    #np.nanmean(variables['PM'])- self.n_sigma*np.nanstd(variables['PM']),\
                        #np.nanmean(variables['PM'])+ self.n_sigma*np.nanstd(variables['PM'])
                                    ...
                                    res=0
                                #print('the fraction of unusual object detected is: {}'.format(res))
                                return res 
                        #else:
                            #print('no objs verify the constrains')
                            #continue
